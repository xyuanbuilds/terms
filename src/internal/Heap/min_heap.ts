export type MinHeapComparator<T> = (curValue: T, comparedValue: T) => number;
 ...
 * @template T 元素类型
 */
export class MinHeap<T extends unknown = number> {
  static defaultComparator = (l: any, r: any) => l - r;

  protected _capacity: number;
  protected _comparator: MinHeapComparator<T>;
  protected _stack: T[] = [];

  constructor(compartor?: MinHeapComparator<T>, capacity?: number) {
    this._capacity = capacity || Number.MAX_SAFE_INTEGER;
    this._comparator = compartor || MinHeap.defaultComparator;
  }
  ...
  // 删除堆顶
  pop() {
    if (this.size === 1) return this._stack.shift();
  ...
  _swap(indexFir: number, indexSec: number) {
    ...
    [this._stack[indexFir], this._stack[indexSec]] = [
      this._stack[indexSec],
      this._stack[indexFir],
    ];
  }
  ...
  _shiftUp(index: number) {
    ...
    if (
      ...
      this._comparator(this._stack[parentIndex], this._stack[index]) > 0
    ) {
  ...
  _shiftDown(index: number) {
    ...
    if (
      ...
      this._comparator(this._stack[leftIndex], this._stack[index]) < 0
    ) {
    ...
    if (
      ...
      this._comparator(this._stack[rightIndex], this._stack[index]) < 0
    ) {
